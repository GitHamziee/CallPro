...
const STATUS_BADGE: Record<string, string> = {
  ...
  EXPIRED: "bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400",
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 sm:p-4">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">Active</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 sm:gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 sm:p-4">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">Expired</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 sm:gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 sm:p-4">
          <div className="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-lg bg-red-50 dark:bg-red-900/30 shrink-0">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">Cancelled</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden">
        ...
        <div className="hidden sm:grid grid-cols-[1fr_140px_100px_100px_90px] gap-3 px-5 py-2.5 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
          ...
          <p className="text-[11px] font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wider text-right">
            Date
          </p>
        </div>
            ...
            {subscriptions.map((sub) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-3 sm:grid sm:grid-cols-[1fr_140px_100px_100px_90px] sm:gap-3 px-3 py-3 sm:px-5 sm:py-4 hover:bg-slate-50/70 dark:hover:bg-slate-700/50 transition-colors group"
                >
                  {/* User */}
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1 sm:hidden">
                        ...
                        {sub.user.accountExecutive && (
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 text-[9px] font-medium truncate max-w-[100px]">
                  ...
                  <div className="hidden sm:flex items-center">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-brand-50 dark:bg-brand-900/30 text-brand-700 text-xs font-medium truncate">
                      {sub.package.name}
                    </span>
                  </div>
                  ...
                  {/* Date — mobile */}
                  <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0 sm:hidden">
                    {timeAgo(sub.createdAt)}
                  </span>
                </Link>
              );
