...
import { CheckCircle, XCircle, ChevronDown, ArrowLeft } from "lucide-react";
import { useSubmitLead } from "@/hooks/useSubmitLead";

const inputClass =
  "w-full px-3 py-2.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-600/20 focus:border-brand-600 transition-all dark:bg-slate-800 dark:text-white";
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-6">
        {editId && (
          <Link
            href="/leads/history"
            className="inline-flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors mb-3"
          >
      ...
      <div className="rounded-xl bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        {loadingLead ? (
          ...
          <form onSubmit={handleSubmit} className="p-4 md:p-6 lg:p-8 space-y-5">
            {message && (
              <div
              ...
              <div>
                ...
                <div className="relative" ref={leadTypeRef}>
                  <button
                    ...
                    className={`flex items-center justify-between w-full px-3 py-2.5 rounded-lg border text-sm transition-all dark:bg-slate-800 ${
                      form.leadType
                  ...
                  >
                    ...
                    <ChevronDown className={`h-4 w-4 text-slate-400 dark:text-slate-500 transition-transform ${leadTypeOpen ? "rotate-180" : ""}`} />
                  </button>
                  {leadTypeOpen && (
                    <div className="absolute top-full left-0 mt-1 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg py-1 z-30">
                      {LEAD_TYPE_OPTIONS.map((opt) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm font-medium transition-colors ${
                            form.leadType === opt.value ? "bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white"
                          }`}
                        >
                          {opt.label}
            ...
            {/* Row 3: Property Type + Beds/Baths + Timeline */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className={labelClass}>
            ...
            {/* Row 4: Contract Status + Appointment Time */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>
                ...
                <div className="relative" ref={contractRef}>
                  ...
                  >
                    ...
                    <ChevronDown className={`h-4 w-4 text-slate-400 dark:text-slate-500 transition-transform ${contractOpen ? "rotate-180" : ""}`} />
                  </button>
                  {contractOpen && (
                    <div className="absolute top-full left-0 mt-1 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg py-1 z-30">
                      {CONTRACT_OPTIONS.map((opt) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm font-medium transition-colors ${
                            form.contractStatus === opt.value ? "bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white"
                          }`}
                        >
                          {opt.label}
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={submitting}
                className="w-full sm:w-auto px-8 py-2.5 bg-brand-600 hover:bg-brand-700 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-colors"
              >
                {submitting
