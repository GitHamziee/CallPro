            ...
            {HOW_IT_WORKS.map((item, i) => (
              <AnimatedSection key={item.step} delay={i * 0.12}>
                <div className="relative flex flex-col items-center text-center">
                  {/* Numbered circle */}
                  <div className="relative z-10 mb-6 flex h-14 w-14 items-center justify-center rounded-full bg-brand-600 shadow-lg shadow-brand-600/30 border-4 border-slate-50 dark:border-slate-900">
                    <span className="text-sm font-bold text-white">{item.step}</span>
                  </div>

                  {/* Card */}
                  <div className="w-full rounded-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
